...
enum FlowState { EXPLORE, LESSON_SELECT, ARTIST_DEMO, PRACTICE, RESULT }
...
var state: int = FlowState.EXPLORE
...
func _on_station_interacted(instrument: String) -> void:
	...
	var station_pos: Vector2 = dan_tranh_station.global_position if instrument == "dan_tranh" else sao_truc_station.global_position
...
func _start_artist_demo(lesson: Dictionary) -> void:
	...
	if state == FlowState.ARTIST_DEMO:
		...
		shell_ui.show_lesson_panel(lesson.get("instrument", "dan_tranh"), _lessons_for_instrument(lesson.get("instrument", "dan_tranh")))
...
func _start_practice(lesson: Dictionary) -> void:
	...
	shell_ui.hide_lesson_panel()
	virtual_artist.stop_demonstration()
	virtual_artist.give_feedback(true, "Your turn. Follow the rhythm lane and keep the tone steady.", 2.2)
...
func _run_practice_simulation(run_id: int) -> void:
	...
	while elapsed < total and state == FlowState.PRACTICE and run_id == practice_run_id:
		...
		practice_ui.update_practice_tick(elapsed, total, event_name, pitch_diff, rolling_accuracy)
...
func _focus_camera(target: Vector2, zoom_value: float) -> void:
	...
	tween.tween_property(camera, "global_position", target, 0.45).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	tween.tween_property(camera, "zoom", Vector2(zoom_value, zoom_value), 0.45).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)

func _show_artist_result_feedback(result: Dictionary) -> void:
	var stars: int = int(result.get("stars", 1))
	if stars >= 3:
		virtual_artist.give_feedback(true, "Excellent control. You kept both rhythm and tone stable.", 3.0)
	elif stars == 2:
		virtual_artist.give_feedback(true, "Good progress. Repeat once more to polish the timing.", 3.0)
	else:
